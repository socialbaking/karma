...
const MAX_REPORT_PROCESSING_MS = TEST_REPORT_PROCESSING_TIMEOUT_MS
  ? +TEST_REPORT_PROCESSING_TIMEOUT_MS
  : 60 * 1000;
...
async function testClient() {
...

  {
    ...
    const pharmacies = organisations.filter(
      (organisation) => organisation.pharmacy
    );
    const onlyPharmacies = organisations.filter(
      (organisation) => organisation.pharmacy && !organisation.clinic
    );
    const otherPartners = organisations.filter(
      (organisation) => !(organisation.clinic || organisation.pharmacy)
    );
...

    {
      ...
      const additionalProductCount =
        isRedis() && TEST_PRODUCT_COUNT ? +TEST_PRODUCT_COUNT : 1;
      const reportCountPerProduct =
        isRedis() && TEST_REPORT_PER_PRODUCT_COUNT
          ? +TEST_REPORT_PER_PRODUCT_COUNT
          : 1;
      ...
      for (
        let productIndex = 0;
        productIndex <= additionalProductCount;
        productIndex += 1
      ) {
        // Complex product
        ...
        const calculated = product.activeIngredients.filter(
          (value) => value.calculated
        );
        // 2 percentage units, 1 multiple unit
        ok(calculated.length >= 3);

        const typeACalculated = calculated.filter(
          (value) => value.type === typeA
        );
        ok(typeACalculated.length >= 1);

        const typeBCalculated = calculated.filter(
          (value) => value.type === typeB
        );
        ok(typeBCalculated.length >= 2);

        for (
          let reportIndex = 0;
          reportIndex <= reportCountPerProduct;
          reportIndex += 1
        ) {
          ...
          const totalCost = items * itemCost + deliveryCost + feeCost;
          ...
          const report = await client.addReport({
            ...
            productItemCost: itemCost.toFixed(2),
            productDeliveryCost: deliveryCost.toFixed(2),
            productTotalCost: totalCost.toFixed(2),
            productFeeCost: feeCost.toFixed(2),
            productItems: items.toString(),
            productOrganisationId: pharmacyPartner.organisationId,
            productOrganisationName: pharmacyPartner.partnerName,
          ...
          if (reportIndex <= 1 && productIndex <= 1) {
            ...
            foundReport = reports.find(
              (value) => value.reportId === report.reportId
            );
            ok(foundReport);
          }
        }
      ...
      const reportsBackgroundTotalProcessing =
        reportsProcessedAt - reportsSubmittedAt;
      ...
      if (reportsBackgroundTotalProcessing > MAX_REPORT_PROCESSING_MS) {
        console.warn(
          `Processing reports took longer than ${MAX_REPORT_PROCESSING_MS}ms`
        );
      }
      ...
      ok(
        reportsBackgroundTotalProcessing <
          MAX_REPORT_PROCESSING_MS * allowedFactor,
        `Processing reports took longer than ${
          MAX_REPORT_PROCESSING_MS * allowedFactor
        }ms`
      );
    }

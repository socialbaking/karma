...

    {
        ...
        fastify.get<Schema>("/authsignal/callback", {
            ...
            async handler(request, response) {
                ...
                const { sub, other: { idempotencyKey, tenantId } } = decoded;
...

    {
        ...
        fastify.post<Schema>("/authsignal/redirect", {
            ...
            async handler(request, response) {
                ...
                const redirectUrl = AUTHSIGNAL_REDIRECT_URL || `${getOrigin()}${fastify.prefix}/authsignal/callback`;
                ...
                const { state, url, idempotencyKey: returnedIdempotencyKey } = await authsignal.track({
                    action: actionCode,
                    userId,
                    email,
                    idempotencyKey,

...
export const importmapRootName: string = (packageInfo.importmap?.root ?? "esnext/imports").replace(":name", name);
export const importmapPrefix: string = (packageInfo.importmap?.prefix ?? ":name").replace(":name", name);

...
export const PRODUCT_UNIT_REGEX =
  /(?:^|\s|[\[(])([≤<]?\s*(?:\d+\.\d+|\d+)\s*[a-z]{1,2}(?:\s*\/\s*[a-z][A-Za-z]?[a-z]*)?)(?:\s|$|,|[\])])/g;
...
export function getActiveIngredients(
  ...
  return descriptions.flatMap((value) =>
    getActiveIngredientsFromString(data, value)
  );
}

function getActiveIngredientsFromString(
  data: Partial<Product>,
  string: string
  ...
  if (percentageMatch) {
    const match = percentageMatch[1].replace(/%$/, "").trim();
  ...
  if (unitMatches) {
    for (const [, unitMatch] of unitMatches) {
      const { unit, value, prefix } = splitValueUnitPrefix(unitMatch);

...
async function seedFromNZULM() {
            ...
            async row => {
                ...
                function parseActiveIngredientDescriptions(): string[] {
                    ...
                    const termIngredients = terms
                        .map(term => {
                            const termRegex = new RegExp(`${term}${PRODUCT_UNIT_REGEX.source}`, "i");
                            const match = workingName.match(termRegex);
                            if (!match) return undefined;
                            return `Total ${NZULM_SEARCH_TERM_ACRONYM[term]} ${replaceUnit(match[1])}`;
                        })
                        .filter(Boolean);
                    ...
                    const acronymIngredients = acronyms
                        .map(acronym => {
                            const acronymRegex = new RegExp(`${acronym}${PRODUCT_UNIT_REGEX.source}`, "i");
                    ...
                    const foundSizedUnit = acronymIngredients
                        .find(value => value.includes("/"));

                    if (acronymIngredients.length === terms.length && (!titleContainsSizedUnit || foundSizedUnit)) {
                        // console.log(productName, acronymIngredients)
                        return acronymIngredients
                    }

                    const unitMatches = [...workingName.matchAll(PRODUCT_UNIT_REGEX)]
                        .map((match) => {
                            return match[1];
                        })
                    ...
                    if (terms.length === 1 && unitMatches.length === 1) {
                        return [
                            `Total ${NZULM_SEARCH_TERM_ACRONYM[terms[0]]} ${replaceUnit(unitMatches[0])}`
                        ]
                    }

                    const types = terms
                        .map(term => NZULM_SEARCH_TERM_ACRONYM[term]);
                ...
                function findOrganisation() {
                    ...
                    return organisations.find(
                        organisation => {
                            if (organisation.organisationName.toLowerCase().includes(lowerFirstWord)) {
                                return true;
                            }
                            return !!organisation.associatedBrandingTerms?.find(
                                term => term.toLowerCase().includes(lowerFirstWord)
                            )
                        }
                    )
                }
    ...
    if (duplicates.size) {
        ...
        for (const duplicateName of duplicates) {
            const duplicateProducts = products.filter(value => value.productName === duplicateName);
            const product: Product = {
                ...duplicateProducts[0],
    ...
    async function getSeedInformation() {
        ...
        for (const { text: genericText, url } of genericLinks) {
            ...
            const $headers = $results
                .children(".row.search_results_subheader_wrapper:not(:first-child)")
                        ...
                        .flatMap<RowInformation>(row => {
                            ...
                            const subsidies = $row.find(".subsidy_cell [title]")
                                ...
                                .map(function (node): ProductInfo {
                                    ...
                                    const title = $node
                                        .attr("title")
                                        .replace("See details below.", "").trim();
    ...
    async function getGenericLinks(criteria: string) {
        ...
        const $genericList = $lists.filter(function (this: Element) {
            ...
            return !!$this.find(".search_results_subheader").text().includes("Generic")
        });

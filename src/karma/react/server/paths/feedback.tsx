import { useError, useMaybeBody, useMaybeResult, useSubmitted } from "../data";
...
function assertFeedback(value: unknown): asserts value is FeedbackFormMetaData {
  ...
  ok(
    value.locatedInNewZealand,
    "We can only accept feedback from people located in New Zealand at this time"
  );
...
export async function submit(request: FastifyRequest) {
  ...
  if (GITHUB_TOKEN) {
    ...
    ok(
      ...
      `Expected homepages origin to include github.com, got ${origin} for ${homepage}`
    );
    const split = repository.split("/").filter(Boolean);
    ok(
      split.length === 2,
      `Expected two parts to homepage pathname, got ${split.join(", ")}`
    );
    ...
    <form name="calculator" action="/feedback#action-section" method="post">
      ...
      <div className="flex flex-col">
        <label className={FORM_GROUP_CLASS}>
          ...
          <input
            ...
            defaultValue={result?.success ? "" : body?.title || ""}
          />
        </label>
        <label className={FORM_GROUP_CLASS}>
          ...
          <input
            ...
            defaultValue={result?.success ? "" : body?.name || ""}
          />
        </label>
        <label className={FORM_GROUP_CLASS}>
          ...
          <textarea
            ...
            defaultValue={result?.success ? "" : body?.message || ""}
          />
        </label>
      </div>
      <hr className="my-8" />
      <label
        htmlFor="locatedInNewZealand"
        className="my-4 flex flex-row items-center"
      >
        <input
          name="locatedInNewZealand"
          id="locatedInNewZealand"
          type="checkbox"
          className="form-checkbox rounded m-1"
          defaultChecked={
            result?.success ? false : body?.locatedInNewZealand || false
          }
        />
        <span className="block ml-4">
          We adhere to the&nbsp;
          <a href={NZCODE_URL} target="_blank" rel="noreferrer">
            New Zealand Code
          </a>
          &nbsp;licence. Because of this, we would like to acknowledge and
          understand if feedback or feature requests are by New Zealanders.
          <br />
          By ticking this checkbox you are confirming you are located in New
          Zealand.
        </span>
      </label>
      <label
      ...
      >
        <input
          ...
          className="form-checkbox rounded m-1"
          defaultChecked={
            result?.success ? false : body?.storageConsent || false
          }
        />
        <ul className="ml-4 list-none divide-y divide-dashed">
          <li>I give consent for the provided information to be stored.</li>
          <li>
            I give consent for the provided information to be published
            publicly, for example on sites like{" "}
            <a href={bugsUrl} target="_blank" rel="noreferrer">
              GitHub
            </a>
          </li>
          <li>
            I give consent for the feedback to be used to implement features
            potentially in more than one codebase, including code bases licenced
            using the{" "}
            <a href={MIT_URL} target="_blank" rel="noreferrer">
              GitHub
            </a>{" "}
            licence or{" "}
            <a href={CC0_URL} target="_blank" rel="noreferrer">
              GitHub
            </a>
          </li>
          <li>
            I understand that the feedback will be taken into consideration and
            may or may not be included as a feature
          </li>
        </ul>
      </label>
      <hr className="my-8" />

...
export function getActiveIngredients(product: Product): ActiveIngredient[] {
    ...
    let calculated = activeIngredients
        .filter(value => (
            value.calculated &&
            value.unit === unit &&
    ...
    return types
        .map(
            (type): ActiveIngredientData => {
                const typeValues = calculated
                    .filter(value => value.type === type);
                const typeUnit = typeValues[0].unit;
                const unitValues = typeValues
                    .filter(value => value.unit === typeUnit);

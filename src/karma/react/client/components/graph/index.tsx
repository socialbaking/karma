...
function getProductsFromMetrics(metrics: CountryProductMetrics, productId: string) {
...
function getXY(props: ProductMetricsProps, metric: ProductValue) {
    ...
    const valueSpan = Math.max(values.length < 1 ? 150 : maxValue - minValue, 150);
...
function getPath(props: ProductMetricsProps, values: ProductValue[]) {
    ...
    return path.map(({ x, y }, index) => `${index ? "L" : "M"} ${x} ${y}`).join(" ");
}
...
export function MetricsProduct(props: ProductMetricsProps) {
    ...
    const timestamps = useMemo(() => {
        return props.timestamps.filter(
            timestamp => !!timestamp.metrics.find(metrics => isProductInMetrics(metrics, productId))
        )
...
export function MetricsGraph(props: MetricsGraphProps) {
    ...
    const {
        ...
        let durationMetrics = metrics.filter(value => value.duration === duration);
        const countryCode = givenCountryCode || durationMetrics[0]?.countryCode || "NZ";
        durationMetrics = durationMetrics.filter(value => value.countryCode === countryCode);
        const reportingDateKey: ReportDateDataKey = durationMetrics[0]?.reportingDateKey || "orderedAt";
        return {
            countryCode,
            reportingDateKey,
    ...
    console.log({ durationMetrics: durationMetrics.length, reportingDateKey,
        countryCode })

    const unit = props.unit || (durationMetrics[0]?.currencySymbol ? `${durationMetrics[0]?.currencySymbol}/g` : undefined) || "$/g";

    const productIds = useMemo(() => (
        durationMetrics
            .flatMap<string>(value => value.products.map(value => value.productId))
    ), [durationMetrics]);
    ...
    const { values, minValue, maxValue } = useMemo(() => {
        const allValues = durationMetrics
            .flatMap<number>(
                value =>
                    value.products
                        .flatMap<number>(
    ...
    const timestamps = useMemo(() => {
        ...
        const indexedDates = durationMetrics
            .map(value => DateTime.fromISO(value[reportingDateKey], { zone: timezone }))

        const dates = indexedDates
            .sort((a, b) => a.diff(b).as("milliseconds"))
        ...
        const last =  unitAfter.diff(datesLast).as("milliseconds") < 0 ?
            datesLast :
        ...
        return Array.from({ length: difference }, (ignore, index): Timestamp => {
            ...
            const metrics = durationMetrics.filter(
                (metric, index) => {
            ...
            const values = metrics.flatMap<ProductValue>(
                metrics => metrics
                    .products
                    .map((product): ProductValue => {

                        const units = product.activeIngredients.filter(isMatchingUnit);
                        const types = [...new Set(units.map(value => value.type))]
                            .sort((a, b) => {
                                if (a === b) return 0;

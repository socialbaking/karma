import {ActiveIngredientMetrics, MetricsData, ProductMetricData} from "../../../client";
...
function isMetricMatch(value: ActiveIngredientMetrics, { numeric, type, unit }: Partial<MetricOptions>) {
...
export function useMetrics(metrics: SingleProductMetrics | undefined, { numeric, type, unit }: MetricOptions): ActiveIngredientMetrics[] {
    return useMemo(() => {
...
export function useMetric(metrics: SingleProductMetrics | undefined, options: MetricOptions) {
    const match = useMetrics(metrics, options);
    return useMetricMatch(match);
}

export function useMetricMatch(match?: ActiveIngredientMetrics[], { numeric, type, unit }: Partial<MetricOptions> = {}) {
    return useMemo(() => {
        const options = { numeric, type, unit };

...
const FILTER_HTTP_WORDS = ["accessToken", "apiToken", "authorization"].flatMap(
  (value) => [value, value.toLowerCase(), value.toUpperCase()]
);
        ...
        "@opentelemetry/instrumentation-fastify": {
          ...
          requestHook(span, info) {
            ...
            url.search = new URLSearchParams(
              Object.entries(query).map(mapEntry)
            ).toString();
            const { origin } = url;
            ...
            function mapEntry([key, value]: [string, string]) {
              if (FILTER_HTTP_WORDS.includes(key))
                return [key, FILTERED_WORD_VALUE];
              return [key, value];
            }
          },
        },

...

{
    ...
    const msSinceCommit = Date.now() - date.getTime();
    const secondsBetweenCommitAndBuild = msSinceCommit / 1000;
    const minutesBetweenCommitAndBuild = secondsBetweenCommitAndBuild / 60;
    const minutesFloored = Math.floor(minutesBetweenCommitAndBuild)
    const secondsWithoutMinutes = Math.floor(secondsBetweenCommitAndBuild - (minutesFloored * 60));
    const secondsTime = secondsWithoutMinutes > 1 ? ` and ${secondsWithoutMinutes} seconds` : "";
    const timeBetweenCommitAndBuild = `${minutesFloored} minutes${secondsTime}`
